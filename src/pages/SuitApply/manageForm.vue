      ...
      <n-form style="width:688px">
        ...
        <n-form-item>
          ...
          <n-card :bordered="false" title="名称" style="width: 275px;height:43px">
            <n-select v-model:value="form.supplies_name" :options="nameOptions"></n-select>
            </n-card>
            <n-card :bordered="false" title="规格" style="width: 275px;height:43px">
              <n-select v-model:value="form.spec" :options="sizeOptions"></n-select>
            </n-card>
          </n-space>
        </n-form-item>
        <n-form-item>
          ...
          <n-card :bordered="false" title="数量" style="width: 275px;height:43px">
            <n-input v-model:value="form.count" type="text" placeholder="请输入数量" />
            </n-card>
        ...
        <n-form-item>
          <n-button type="primary" style="left: 19px;top: 37px;" @click="check">
            同意
          </n-button>
          <n-button type="error" secondary style="left: 39px ;top: 37px" @click="reject">
            否决
          </n-button>
          <n-button type="info" third style="left: 59px ;top: 37px" @click="change">
            确认修改
          </n-button>
        </n-form-item>
      </n-form>
          ...
          if(resData[i].campus == (form.campus === "朝晖" ? 1 : (form.campus==="屏峰" ? 2 : 3))){
            if(resData[i].name == form.supplies_name){
              for(let j=0; j<resData[i].specs.length;j++){
                sizeOptions.push({label:resData[i].specs[j].spec,value:resData[i].specs[j].id});
...
watch(
  () =>[form.campus,form.supplies_name],
  () => {
  console.log(form.campus);
  resetForm();
  console.log(form);
  updateName();
  updateSize();
