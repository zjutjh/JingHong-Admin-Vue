<script setup lang="ts">
  ...
  import { NSpace,NLayout,NInput,NButton,NTable,NDatePicker,NModal,NCard } from 'naive-ui'
  ...
  const deleteFlag =ref(false);//删除弹窗判值
...
<n-space >
  <n-layout id="layout1">  
    ...
    <n-space space-around>  
       ...
       <n-space>  
         <n-button type="success"  id="Send" @click="onClickSend">发送</n-button>
         <n-button type="warning" id="Clear" @click="onClickClear">清空</n-button>
       </n-space>
      </n-space>
    ...
    <n-space vertical>    
      ...
      <n-table>
          ...
          <tbody>
            <tr v-for="Announcement in Announcements">
              ...
              <td><n-button  type="success"  size="small"
              @click="()=> onClickSelect(Announcement.id, Announcement.title, Announcement.content)">编辑</n-button></td>
              <td><n-button type="error" size="small" @click="()=>{deleteFlag=true}">删除</n-button></td>

              <n-modal v-model:show="deleteFlag">
              <n-card
              title="是否确认删除:"
              style="width: 200px"
              :bordered="false"
               ...
               aria-modal="true"
               >
              <n-button type="error" @click="onClickDelete(Announcement.id)">确认</n-button>
              <n-button @click="()=>{deleteFlag=false}" type="success" id="return">取消</n-button>
             </n-card>
             </n-modal>
     ...
     <n-space vertical> 
      ...
      <n-button type="success" @click="()=> onClickUpdate()">提交</n-button>
      </n-space>
